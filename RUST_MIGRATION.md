...
| Brancher `Provable`/`constraintSystem()` d'o1js sur l'adaptateur | 🟨 | **preuve Pickles Rust depuis du vrai code o1js** : `rust-pickles-recorded.ts` intercepte `Snarky.field.*`/`gates.generic` en mode witness-gen (valeurs via readVar/asProver), enregistre le circuit en JSON RecordedCircuit et le prouve via `rust_pickles_prove_recorded_base/_n1` (@o1js/native). Smoke : `./run src/tests/rust-pickles-recorded.ts` — base 3.7s, N1 12.4s, verify standalone. Recorders poseidon/EC/range-check FAITS ; **intégration ZkProgram FAITE (expérimental)** : `Program.experimentalRustPickles.{proveBaseCase, proveBaseCaseKeep, proveN1, proveN1Over, verifyBaseCase, verifyN1}` — `proveBaseCaseKeep`+`proveN1Over` = chaînage SelfProof réel (la preuve de l'appel k, gardée dans un handle natif opaque, est vérifiée récursivement par l'appel k+1 qui exécute une autre méthode ; chaîné en 9.4s, falsification rejetée). Tests : `rust-pickles-recorded/zkprogram/chained.ts`. Reste : sérialisation du handle, recorded N2 |
| Parité de VK : circuits o1js compilés via natif == via js_of_ocaml | 🟨 | `rust-pickles-vk-parity.ts` vérifie maintenant en dur les invariants sérialisation/codec : VK jsoo base64 ⇄ base58 canonique, VK Rust base58 ⇄ base64 canonique, metadata identiques, proof JSON Rust roundtrip parser o1js, verify standalone + tamper rejection. Les commitments restent non iso (0/28), donc compatibilité on-chain pas encore acquise. |
| **Porter Pickles en Rust** (step/wrap, side-loaded keys, feature flags) | 🟨 | crate `pickles` : base-case + récursion N1/N2 e2e, verify standalone side-loaded (`verify.rs`), circuits enregistrés pilotés depuis JS (`recorded.rs`) exposés NAPI+WASM ; récursion pilotée depuis JS FAITE (chaînage via handle natif) ; **parité VK/gates vs jsoo EN COURS** (critère on-chain) : rapport VK strict sur codecs/métadonnées (`rust-pickles-vk-parity.ts`) — metadata OK, commitments 0/28 ; diff gate-level du step (`rust-pickles-step-gates-diff.ts`, dump jsoo via interception cache step-pk + `prover_to_json`) — 512 rows des deux côtés, sponge identique, histogrammes de gates identiques après port du préambule `dummy_constraints()` o1js (Generic 89, Poseidon 319, Zero 98, CompleteAdd 3, VarBaseMul 1, EndoMul 1, EndoMulScalar 1) ; reste : aligner 10 wirings/coefficients internes du dummy préambule, puis le wrap (nécessite `fq_prover_to_json`) ; feature flags |
