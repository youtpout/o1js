...
| **Porter Pickles en Rust** (step/wrap, side-loaded keys, feature flags) | 🟨 | crate `pickles` : base-case + récursion N1/N2 e2e, verify standalone side-loaded (`verify.rs`), circuits enregistrés pilotés depuis JS (`recorded.rs`) exposés NAPI+WASM ; récursion pilotée depuis JS FAITE (chaînage via handle natif) ; **parité VK/gates vs jsoo EN COURS** (critère on-chain) : rapport VK strict sur codecs/métadonnées (`rust-pickles-vk-parity.ts`) — metadata OK, commitments 0/28 ; diff gate-level du step (`rust-pickles-step-gates-diff.ts`, dump jsoo via interception cache step-pk + `prover_to_json`) — 512 rows des deux côtés, sponge identique, Generic 86 vs 89 après ajout des checks on-curve des points VK ; reste : bloc EC 8 rows OCaml non identifié, ordre des rows, wiring, puis le wrap (nécessite `fq_prover_to_json`) ; feature flags |
