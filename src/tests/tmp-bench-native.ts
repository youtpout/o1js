/**
 * Native benchmark: 3-branch recursive ZkProgram (init / update / merge),
 * timed compile (Cache.None) + proofs N0/N1/N2 + verify, printing the VK.
 *
 * BENCH_BACKEND=rust  -> native addon + rust proof-system (mina-runtime)
 * BENCH_BACKEND=jsoo  -> default OCaml/jsoo pipeline
 *
 * Run: BENCH_BACKEND=rust ./run src/tests/tmp-bench-native.ts
 */
import {
  Cache,
  Field,
...
if (process.env.BENCH_SKIP_PROVE === '1') {
  ...
  console.log(
    `VK data (${verificationKey.data.length} chars): ${verificationKey.data.slice(0, 72)}...`
  );
  process.exit(0);
}
...
// the regular rust N2 path currently requires two retained BASE proofs
t = performance.now();
...
console.log(`TOTAL compile+3 proofs+verify: ${ms(tCompile + tInit + tUpdate + tInit2 + tMerge + tVerify)}`);
console.log(`VK hash: ${verificationKey.hash.toString()}`);
console.log(`VK data (${verificationKey.data.length} chars): ${verificationKey.data.slice(0, 72)}...`);
