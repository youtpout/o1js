...
const JSOO_DUMP = `${DUMP_DIR}/sl-gates-jsoo.json`;
const BRANCHES_DUMP = `${DUMP_DIR}/sl-branches.json`;

import { DynamicProof, SmartContract, VerificationKey, method } from '../index.js';

class SideLoadedChildProof extends DynamicProof<Field, Field> {
  static publicInputType = Field;
  static publicOutputType = Field;
  static maxProofsVerified = 0 as const;
}

class SideLoadedZkapp extends SmartContract {
  @method async check(proof: SideLoadedChildProof, vk: VerificationKey) {
    proof.verify(vk);
...
if (mode === 'jsoo') {
  ...
  console.log('compiling the SideLoaded zkapp via jsoo Pickles (capturing pks)...');
  await SideLoadedZkapp.compile({ cache: interceptCache as any, forceRecompile: true });
  ...
  console.log('compiling the SideLoaded zkapp via the rust backend (dumping branches)...');
  await SideLoadedZkapp.compile({ forceRecompile: true });
  ...
  writeFileSync(`${DUMP_DIR}/sl-gates-rust.json`, JSON.stringify(rustProgram));
  ...
  let methods: [string, number][] = [['check', branchesMeta[0]?.proofsVerified ?? 0]];
  let allMatch = true;
