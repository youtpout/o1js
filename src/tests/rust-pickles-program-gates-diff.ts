...
const JSOO_DUMP = `${DUMP_DIR}/program-gates-jsoo.json`;
const BRANCHES_DUMP = `${DUMP_DIR}/program-branches.json`;
...
if (mode === 'jsoo') {
  ...
  console.log('compiling the Add program via jsoo Pickles (capturing pks)...');
  ...
  console.log('compiling the Add program via the rust backend (dumping branches)...');
  ...
  writeFileSync(`${DUMP_DIR}/program-gates-rust.json`, JSON.stringify(rustProgram));
